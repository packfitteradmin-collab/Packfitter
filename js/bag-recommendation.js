...
(function () {
  ...
  // Generate stable slug IDs for each bag (used for PAGE_STATIC_PRODUCT_IDS dedup)
  ...
  BAG_DB.forEach(function(b) { b.slug = slugify(b.name); });
  ...
  function recommendBags(opts) {
    ...
    // Filter out products already shown as static editorial cards on this page
    var staticIds = window.PAGE_STATIC_PRODUCT_IDS || [];
    if (staticIds.length > 0) {
      function isStatic(bag) { return bag && staticIds.indexOf(bag.slug) !== -1; }
      if (isStatic(result.safest)) {
        // Promote first alsoWorks to safest, or null
        result.safest = result.alsoWorks.length > 0 ? result.alsoWorks.shift() : null;
      }
      result.alsoWorks = result.alsoWorks.filter(function(b) { return !isStatic(b); });
      if (isStatic(result.stricterOption)) {
        result.stricterOption = null;
      }
    }
    ...
    if (!result.safest && result.alsoWorks.length === 0 && !result.stricterOption) {
      // No valid recommendations after dedup — leave container empty, card stays hidden
      container.innerHTML = "";
      return result;
    } else {
      // A. Best Match
    ...
    // Show the parent card only if there are valid recommendations to display
    var parentCard = document.getElementById("bagRecommendationsCard");
    var hasRecs = result.safest || result.alsoWorks.length > 0 || result.stricterOption;
    if (parentCard && hasRecs) {
